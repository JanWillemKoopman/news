  ...
  return (
    <div className="space-y-3 border-b border-neutral-100 pb-4">
        ...
        {runs.map((run) => {
          ...
          return (
            <li key={run.id}>
              ...
              >
                <span className="flex-1 text-neutral-800">{dateLabel(run.created_at)}</span>
                <span className="text-xs text-neutral-500">
                  R² {fmt(run.summary.diagnostics.r2, 2)} · MAPE {pct(run.summary.diagnostics.mape)}
                </span>
                <span className={`text-xs font-medium ${v.rose ? "text-rose-600" : "text-neutral-400"}`}>
                  {v.text}
                </span>
                {run.is_published && (
      ...
      {compareMode && compareRuns.length < 2 && (
        <p className="text-xs text-neutral-400">Kies twee runs hierboven om te vergelijken.</p>
      )}
    </div>
  );
}
    ...
    <div className="overflow-x-auto rounded-lg border border-neutral-200">
      <table className="w-full min-w-[480px] text-sm">
        ...
        <tbody className="divide-y divide-neutral-100">
          <tr>
            <td className="p-2 text-neutral-500">R²</td>
            {runs.map((r) => (
              <td key={r.id} className="p-2 font-medium text-neutral-900">
                {fmt(r.summary.diagnostics.r2, 2)}
              </td>
            ))}
          </tr>
          <tr>
            <td className="p-2 text-neutral-500">MAPE</td>
            {runs.map((r) => (
              <td key={r.id} className="p-2 font-medium text-neutral-900">
          ...
          <tr>
            <td className="p-2 text-neutral-500">Max R-hat</td>
            {runs.map((r) => (
              <td key={r.id} className="p-2 font-medium text-neutral-900">
          ...
          {channelNames.map((name) => (
            <tr key={name}>
              <td className="p-2 text-neutral-500">{name} — aandeel</td>
              {byName.map((m, i) => {
                const ch = m.get(name);
                return (
                  <td key={i} className="p-2 font-medium text-neutral-900">
