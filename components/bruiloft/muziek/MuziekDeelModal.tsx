...
interface MuziekDeelModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

// Muzieklijst delen met de DJ of band: één publieke link per bruiloft.
// Delen aan = link bestaat; "Stop met delen" maakt de link per direct
// ongeldig. Zelfde deel-idioom als DraaiboekDeelModal (linkveld + kopieer +
// WhatsApp) — de ontvanger ziet alleen de goedgekeurde nummers.
export function MuziekDeelModal({ open, onOpenChange }: MuziekDeelModalProps) {
  const wedding = useBruiloftStore((s) => s.wedding)
  const share = useBruiloftStore((s) => s.muziekShare)
  const enableMuziekShare = useBruiloftStore((s) => s.enableMuziekShare)
  const disableMuziekShare = useBruiloftStore((s) => s.disableMuziekShare)
  ...
  const link = share && origin ? `${origin}/muziek/${share.token}` : null
  const namen = wedding
    ? [wedding.partner1Naam, wedding.partner2Naam].filter(Boolean).join(' & ')
    : ''
  const bericht = link
    ? `Hoi! Hierbij de muzieklijst voor onze bruiloft${namen ? ` (${namen})` : ''}: ${link}`
    : ''
  ...
  const copyLink = async () => {
    ...
    try {
      ...
      setCopied(true)
      toast({ title: 'Link gekopieerd', description: 'De muzieklijst-link staat nu in je klembord.', variant: 'success' })
  ...
  return (
    <Modal
      ...
      title="Muzieklijst delen"
      description="Voor jullie DJ of band — die ziet de nummers per moment van de dag zonder in te loggen."
      ...
      {share ? (
        <div className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            Iedereen met deze link ziet de meest actuele versie — voeg je een
            nummer toe, dan ziet de DJ dat vanzelf. Nog niet beoordeelde
            gastsuggesties blijven privé. De ontvanger kan de lijst printen of
            als tekst kopiëren.
          </p>
        ...
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            ...
            ontvanger heeft geen account nodig en ziet alléén de muzieklijst —
