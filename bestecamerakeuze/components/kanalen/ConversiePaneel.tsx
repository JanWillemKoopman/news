...
import HerkomstPaneel, { type Herkomst } from "@/components/kanalen/HerkomstPaneel";
...
export default function ConversiePaneel() {
  ...
  // Het herkomstoverzicht komt uit dezelfde aanroep en hoort ná elk vinkje opnieuw op te
  // halen: aanwijzen verandert precies de cijfers die dat blok laat zien.
  ...
  const haal = useCallback(() => {
    ...
    fetch("/api/kanalen/conversies")
      .then(async (res) => {
        ...
        setHerkomst(data.herkomst ?? null);
        setFout(null);
      })
  ...
  async function bewaar(actie: ConversieActie, patch: Partial<ConversieActie>) {
    ...
    try {
      ...
      // Alleen het herkomstblok bijwerken, niet de hele lijst: die staat hierboven al
      // goed gezet, en hem opnieuw vullen laat het vinkje waar je net op klikte
      // zichtbaar terugspringen.
      fetch("/api/kanalen/conversies")
        .then((r) => (r.ok ? r.json() : null))
        .then((d: { herkomst?: Herkomst } | null) => {
          if (d?.herkomst) setHerkomst(d.herkomst);
        })
        .catch(() => {});
    } catch (err) {
  ...
  return (
    <>
      {/* Eerst zien wat er staat, dan pas instellen: het overzicht is de aanleiding
          om iets aan te vinken, dus het hoort erboven. */}
      <HerkomstPaneel herkomst={herkomst} />
