...
export function vendorFromRow(r: Tables['vendors']['Row']): Vendor {
  return {
    ...
    // any: business_id ontbreekt nog in de gegenereerde database.types.ts.
    businessId: (r as any).business_id ?? undefined,
  }
}
...
export function vendorToRow(p: Partial<VendorInput>): Partial<Tables['vendors']['Insert']> {
  ...
  if (p.businessId !== undefined) (r as any).business_id = p.businessId ?? null
  return r
}
