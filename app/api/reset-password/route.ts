...
export async function POST(request: Request) {
  ...
  if (verifyError || !verifyData.user) {
    // Guardrail: log de gestructureerde GoTrue-fout (name/status/code) zodat de
    // échte oorzaak — verlopen token vs. config-/projectfout — meteen in de logs
    // zichtbaar is. Naar de gebruiker tonen we bewust een generieke melding.
    const e = verifyError as
      | { name?: string; message?: string; status?: number; code?: string }
      | null
    console.error(
      '[reset-password] verifyOtp mislukt:',
      JSON.stringify({
        name: e?.name,
        status: e?.status,
        code: e?.code,
    ...
    return NextResponse.json({ error: 'Email link is invalid or has expired' }, { status: 400 })
  }
