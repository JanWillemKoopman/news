...
export async function POST(request: NextRequest) {
  ...
  const fingerprint = buildTakenFingerprint(body.context, bestaandeTaken)
  const now = Date.now()
  ...
  if (cacheRow) {
    ...
    if (cacheAge < MAX_CACHE_AGE_MS && (cacheRow.data_fingerprint === fingerprint || cacheAge < MIN_COOLDOWN_MS)) {
      ...
      return NextResponse.json({
        advies: cacheRow.cached_advies as AITakenAdvies,
        cached: true,
  ...
  const rl = await checkRateLimit(`ai:taken:${body.weddingId}`, 1, 60 * 60)
  if (!rl.allowed) {
