...
def generate_with_chunks(
    ...
    """Identiek aan generate(), maar retourneert ook grounding chunk URIs van Google Search."""
    url = f"{_BASE}/{model}:generateContent?key={GEMINI_API_KEY}"
    ...
    try:
        ...
        for chunk in candidate.get("groundingMetadata", {}).get("groundingChunks", []):
            ...
            if uri:
                grounding_uris.append(uri)

        return {"text": text, "grounding_uris": grounding_uris}
    except (KeyError, IndexError) as e:
        raise RuntimeError(f"Onverwacht Gemini-antwoord: {str(data)[:500]}") from e
